...
<?php foreach ($this->Migration->columns($table) as $column => $config): ?>
            ...
            ?>])
<?php endforeach; ?>
<?php foreach ($this->Migration->constraints($table) as $constraint):
...
<?php elseif ($constraintColumns !== $primaryKeysColumns): ?>
            ->addIndex(
                [<?php
...
<?php endif; ?>
<?php endforeach; ?>
<?php foreach($this->Migration->indexes($table) as $index):
            sort($foreignKeys);
...
<?php endforeach; ?>
    ...
    {
<?php $tables = array_reverse($tables);
